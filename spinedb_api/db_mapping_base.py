...
class _MappedTable(dict):
    ...
    def unique_key_value_to_id(self, key, value, strict=False, fetch=True):
        ...
        if not id_ and fetch:
            id_ = self._db_map.fetch_value(self._item_type, _get_id)
        if strict and id_ is None:
            raise KeyError(key)
        return id_
    ...
    def _add_unique(self, item):
        ...
        # entity{"id": 3, "class_name": "dog", "name": "Scooby"}
        # entity{"id": 4, "class_name": "dog", "name": "Pluto"}
